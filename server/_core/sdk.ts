...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Accounts created by this application are verified locally, without the
    // private Manus OAuth API.
    if (session.openId.startsWith("local_")) {
      const user = await db.getUserByOpenId(session.openId);
      if (!user) throw ForbiddenError("Local account not found");
      return user;
    }
